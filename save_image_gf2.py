...
def generate_data():
    ...
    original_pan = np.expand_dims(np.float32(np.load(pan_path)), dim=0) / 1024.0 #
    ...
    return original_hs, original_pan


def crop_data(hs_data, pan_data):
    train_ratio = 0.8
    train_factor = 1
    ...
    train_hs_image, train_pan_image = \
        Crop_traindata_two(hs_data,             # 可调整size
                            pan_data,
                            training_size,
    ...
    return train_hs_image, train_pan_image
