...
def generate_data():
    ...
    hs_path = '/home/aistudio/data/data136740/msi.npy'
    pan_path = '/home/aistudio/data/data136740/pan.npy'
    # save_dir = "/home/aistudio/work/wuhan_wv2"

    original_hs = np.float32(np.load(hs_path)) / 1024.0 #[:, :1000, :1000]

    original_hs = original_hs[:, :, 2:-2]
    original_hs = original_hs[:, :int(original_hs.shape[1]-original_hs.shape[1]%64), :int(original_hs.shape[2]-original_hs.shape[2]%64)]
    original_hs = original_hs[:, :-1000, :-1000]
 
    original_pan = np.expand_dims(np.float32(np.load(pan_path)), dim=0) / 1024.0 #[:, :1000*12, :1000*12]
    ...
    original_pan = original_pan[:, :-4000, :-4000]
