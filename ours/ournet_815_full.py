...
def ournet(train_ms_image, train_pan_image,
           test_ms_image, test_pan_image, num_epochs=50,
           mid_ch=20, batch_size=10, learning_rate = 1e-3,
           ratio=4, name='wv2'):
    # 影像维数
    # hs_data是真反射率数据
    num_epochs = 50 if name == 'gf2' else 50
    ...
    # torch.seed(1500)
    #  定义数据和模型
    dataset0 = Mydata(train_ms_image, train_pan_image)
    train_loader = torch.utils.data.DataLoader(dataset0, num_workers=0, batch_size=batch_size,
                                        shuffle=True, drop_last=True)

    dataset1 = Mydata(test_ms_image, test_pan_image)
    ...
    ssim_pan = SSIM(window_size=11, size_average=True, channel=1)    # ssim_loss = SSIMLoss(window_size=11, sigma=1.5, data_range=1, channel=band)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (ms, pan) in enumerate(train_loader):
            optimizer.zero_grad()
        ...
        if ((epoch + 1) % 10) == 0:
            ...
            model.eval()
            with torch.no_grad():
                for (images_ms, images_pan) in test_loader:
                    outputs_temp = model(images_ms, images_pan)
                    D_lambda0 += D_lambda(outputs_temp[0].squeeze(0).permute(1,2,0).cpu().numpy(), images_ms.squeeze(0).permute(1,2,0).cpu().numpy())
                    D_s0 += D_s(outputs_temp[0].squeeze(0).permute(1,2,0).cpu().numpy(), images_ms.squeeze(0).permute(1,2,0).cpu().numpy(), images_pan.squeeze(0).permute(1,2,0).numpy())
            print('epoch %d of %d, using time: %.2f , loss of train: %.4f, D_lambda: %.4f, D_s: %.4f' %
                  (epoch + 1, num_epochs, time.time() - time0, loss_total, D_lambda0 / num_patch, D_s0 / num_patch))

            # loss_save_test.append(loss_total_test)
        loss_save.append(loss_total)

    torch.save(model.state_dict(), '/home/aistudio/result/parameters/ournet_model_full_'+str(num_epochs)+name+'.pth')

    # 测试模型
    model_dict =torch.load('/home/aistudio/result/parameters/ournet_model_full_'+str(num_epochs)+name+'.pth')    # 加载训练好的模型参数
    model.set_state_dict(model_dict)
    ...
    with torch.no_grad():
        ...
        time_all = []
        for (images_hs, images_ms) in test_loader:
            time0 = time.time()
