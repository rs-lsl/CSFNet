...
def generate_srf(filename4):  # 产生和每个高光谱波段
    ...
    srf_simu = np.expand_dims(np.zeros(6), dim=0)
    for i0 in range(len(band_center)):
        index = np.where((srf[:, 0] >= left_band[i0]) & (srf[:, 0] <= right_band[i0]))
        srf_simu = np.concatenate((srf_simu, np.mean(srf[index, 1:], dim=1)), dim=0)

    band_center = np.expand_dims(band_center, dim=0)
    print(srf_simu.shape)
    srf_simu = np.concatenate((np.transpose(band_center), srf_simu[1:, :]), dim=1)
...
def generate_data():
    ...
    # srf_pan = np.expand_dims(np.expand_dims(np.load('/home/aistudio/data/data96268/srf_pan.npy'), dim=-1), dim=-1)
    # srf_pan = np.expand_dims(np.expand_dims(np.load('/home/aistudio/work/Chikusei/srf_pan_Chikusei.npy'), dim=-1), dim=-1)

    # Downsampler12 = Downsampler(4, padding=4)

    hs_path = '/home/aistudio/data/data122836/msi.npy'
    pan_path = '/home/aistudio/data/data122836/pan.npy'
    save_dir = "/home/aistudio/work/wuhan_wv2"


    original_hs = np.float32(np.load(hs_path) / 10000.0)#[:, :1000, :1000]
    index_n = np.where(original_hs < 0)
    original_hs[index_n] = 0
    index_n = np.where(original_hs > 1)
    original_hs[index_n] = 1
    original_hs = original_hs[:, :int(original_hs.shape[1]-original_hs.shape[1]%64), :int(original_hs.shape[2]-original_hs.shape[2]%64)]
 
    original_pan = np.expand_dims(np.float32(np.load(pan_path) / 2047.0), dim=0)#[:, :1000*12, :1000*12]
    index_n = np.where(original_pan < 0)
    original_pan[index_n] = 0
    index_n = np.where(original_pan > 1)
    original_pan[index_n] = 1
    original_pan = original_pan[:, :int(original_pan.shape[1]-original_pan.shape[1]%(64*4)), :int(original_pan.shape[2]-original_pan.shape[2]%(64*4))]

    # np.save(save_dir+'/ms_2m.npy', original_hs)
    # np.save(save_dir+'/pan_05m.npy', original_pan)
    print(original_hs.shape)
    print(original_pan.shape)
        
    # temp_blur_hs = Downsampler12(torch.tensor(np.expand_dims(original_hs, dim=0), dtype='float32'))
    # print(temp_blur_hs.shape)
    # temp_blur_hs = np.squeeze(temp_blur_hs.numpy(), dim=0)

    # temp_blur_pan = Downsampler12(torch.tensor(np.expand_dims(original_pan, dim=0), dtype='float32'))
    # print(temp_blur_pan.shape)
    # temp_blur_pan = np.squeeze(temp_blur_pan.numpy(), dim=0)

    # np.save(save_dir+'/ms_8m.npy', temp_blur_hs)
    # np.save(save_dir+'/pan_2m.npy', temp_blur_pan)
    return original_hs, original_pan    # , temp_blur_hs, temp_blur_pan


def crop_data(original_hs, original_pan):
    train_ratio = 0.8
    train_factor = 1
    ...
    testing_size = 64  # testing patch size
    ratio = 16
    ...
    # temp_blur_hs = np.float32(np.load('/home/aistudio/work/wuhan_wv2/ms_8m.npy'))
    # temp_blur_pan = np.float32(np.load('/home/aistudio/work/wuhan_wv2/pan_2m.npy'))
    # original_hs = np.float32(np.load('/home/aistudio/work/wuhan_wv2/ms_2m.npy'))

    # original_hs = np.float32(np.load('/home/aistudio/work/wuhan_wv2/ms_2m.npy'))
    # original_pan = np.float32(np.load('/home/aistudio/work/wuhan_wv2/pan_05m.npy'))

    # train_hs_image, train_pan_image, train_label = \
    #     Crop_traindata_three(temp_blur_hs,             # 可调整size
    #                         temp_blur_pan,
    #                         original_hs,
    #                         training_size,
    #                         test=False, step_facter=1, ratio=4)

    test_hs_image, test_pan_image = \
        Crop_traindata_two(original_hs,
                        original_pan,
                        testing_size,
                        test=True, step_facter=1, ratio=4)

    # print('train size:' + str(train_hs_image.shape))
    print('test size:' + str(test_hs_image.shape))

    # np.save('/home/aistudio/work/'+region+'/train_ms.npy', train_hs_image)
    # np.save('/home/aistudio/work/'+region+'/train_pan.npy', train_pan_image)
    # np.save('/home/aistudio/work/'+region+'/train_label.npy', train_label)

    # np.save('/home/aistudio/work/'+region+'/test_ms.npy', test_hs_image)
    # np.save('/home/aistudio/work/'+region+'/test_pan.npy', test_pan_image)
    return test_hs_image, test_pan_image

def Crop_traindata_two(image_ms, image_pan, size, test=False, step_facter=1, ratio=4):
    image_ms_all = []
    image_pan_all = []
    ...
    for j in range(0, image_ms.shape[1] - size, int(size/step_facter)):
        for k in range(0, image_ms.shape[2] - size, int(size/step_facter)):
            ...
            if all_valid(temp_image_ms, dim=0):
...
def Crop_traindata_three(image_ms, image_pan, image_label, size, test=False, step_facter=1, ratio=4):
    image_ms_all = []
    image_pan_all = []
    label = []
...
def unsupervise_crop():
    ...
    for i2 in range(img_num):
        ...
        pan_data.append(np.expand_dims(np.float32(np.load('/home/aistudio/data/data95831/pan_ziyuan.npy')[:1000*12, :1000*12]) / 4095.0, dim=0))
...
def generate_patch_ms(train_num=1000, ms_band=4):
    ...
    for i2 in range(ms_band):  # worldview2 SRF range of pan include five ms bands
        ...
        result.append(np.expand_dims(np.sum(test_data * \
                                            srf_simu_expand0, dim=1) / np.sum(srf_simu_expand0), dim=1))
    result = np.concatenate(result, dim=1)
...
if __name__ == '__main__':
    ...
    # # print(np.sum(srf_simu, dim=1).shape)
    # print(np.tile(np.expand_dims(np.sum(srf_simu, dim=1), dim=1), [1, 5]))
    # # print(srf_pan.shape)
    # print(hs.shape)
    # print(ms.shape)
    ...
    # endmember_hs2 = np.dot(srf_simu, endmember_hs) / np.tile(np.expand_dims(np.sum(srf_simu, dim=1), dim=1), [1, 5])
